...
function normalizeProduct(product) {
  ...
  const normalizedCatalogType = isPreorderProduct(product) ? "NEW" : normalizeCatalogType(product.catalogType);
  ...
  if (Array.isArray(product.variants) && product.variants.length) {
    return {
      ...
      variants: product.variants.map((variant) => {
        const variantOverride = stockOverrides[getVariantOverrideKey(product.id, variant.id)];
...
export const fetchPreorderProducts = createAsyncThunk("products/fetchPreorder", async (_, { rejectWithValue }) => {
  try {
    const response = await api.get("/products");
    const products = Array.isArray(response.data) ? response.data : [];

    return products.map(normalizeProduct).filter((product) => isPreorderProduct(product));
  } catch (error) {
    return rejectWithValue(error.response?.data?.message || "Không thể lấy danh sách sản phẩm đặt trước");
  }
});
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchProducts.fulfilled, (state, action) => {
        state.loading = false;
        state.products = action.payload.map((product) => applyStockOverridesToProduct(product, state.stockOverrides));
      })
      ...
      .addCase(fetchProductById.fulfilled, (state, action) => {
        state.loading = false;
        state.selectedProduct = applyStockOverridesToProduct(action.payload, state.stockOverrides);
      })
